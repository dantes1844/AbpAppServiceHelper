...
namespace MadsKristensen.AddAnyFile
{
    public static class ProjectHelpers
    {
        ...
        public static ProjectItem AddDirectoryToProject(this Project project, DirectoryInfo directoryInfo, string itemType = null)
        {
            if (project.IsKind(ProjectTypes.ASPNET_5, ProjectTypes.SSDT))
            {
                return _dte.Solution.FindProjectItem(directoryInfo.FullName);
            }

            string root = project.GetRootFolder();

            if (string.IsNullOrEmpty(root) || !directoryInfo.FullName.StartsWith(root, StringComparison.OrdinalIgnoreCase))
            {
                return null;
            }

            ProjectItem item = project.ProjectItems.AddFromDirectory(directoryInfo.FullName);
            item.SetItemType(itemType);
            return item;
        }
        ...
        public static IEnumerable<Project> GetChildProjects(Project parent)
        {
            try
            {
